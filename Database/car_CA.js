...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        const engineResponse = await fetch('options/car_CA_engine_size_values.txt');
        const engineText = await engineResponse.text();
        const engineOptions = engineText.split('\n').filter(engine => engine.trim());

        const transmissionResponse = await fetch('options/car_CA_transmission_values.txt');
