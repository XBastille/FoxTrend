...
class StockDataVisualizer:
    ...
    def plot_historical_data(self, start_date=None, end_date=None):
        ...
        if price_change > 0:
            ...
            print(f"{price_change:.2f}  ({percentage_change:.2f}%)")
        base_path = "public/csv_1"
        file_index = self.index
        while os.path.exists(f"{base_path}/stock_data_{file_index}.csv"):
            file_index += 1
        os.makedirs(base_path, exist_ok=True)
        df.to_csv(f"{base_path}/stock_data_{file_index}.csv")
    ...
    def plot_technical_indicators(self, start_date=None, end_date=None):
        ...
        base_path = "public/csv_1"
        file_index = self.index
        while os.path.exists(f"{base_path}/technical_indicators_{file_index}.csv"):
            file_index += 1
        os.makedirs(base_path, exist_ok=True)
        df[["SMA50", "SMA200", "RSI", "MACD", "MACD_signal", "MACD_histogram", "Bollinger_hband", "Bollinger_lband"]].to_csv(f"{base_path}/technical_indicators_{file_index}.csv")
        '''plt.figure(figsize=(14,7))
