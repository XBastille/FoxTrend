...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        const provinceResponse = await fetch('../public/options/house_CA_Province_values.txt');
        const provinceText = await provinceResponse.text();
        const provinceOptions = provinceText.split('\n').filter(province => province.trim());

        const cityResponse = await fetch('../public/options/house_CA_city_values.txt');
...
document.getElementById('prediction-form').addEventListener('submit', async function (e) {
    ...
    let median_income = document.getElementById('median_income').value
    const price = await fetchingalldata(bedrooms, bathrooms, population, province, city, median_income)
...
let pricing;
async function fetchingalldata(bedrooms, bathrooms, population, province, city, median_income) {
    ...
    try {
        const response = await fetch('/house/CA', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ bedrooms, bathrooms, population, province, city, median_income })
        });
