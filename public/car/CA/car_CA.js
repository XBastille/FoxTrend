...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        const makeResponse = await fetch('../public/options/car_CA_make_values.txt');
        const makeText = await makeResponse.text();
        const makeOptions = makeText.split('\n').filter(make => make.trim());

        const modelResponse = await fetch('../public/options/car_CA_model_values.txt');
        const modelText = await modelResponse.text();
        const modelOptions = modelText.split('\n').filter(model => model.trim());

        const bodyResponse = await fetch('../public/options/car_CA_body_type_values.txt');
        const bodyText = await bodyResponse.text();
        const bodyOptions = bodyText.split('\n').filter(body => body.trim());

        const fuelResponse = await fetch('../public/options/car_CA_fuel_type_values.txt');
        const fuelText = await fuelResponse.text();
        const fuelOptions = fuelText.split('\n').filter(fuel => fuel.trim());

        const drivetrainResponse = await fetch('../public/options/car_CA_drivetrain_values.txt');
        const drivetrainText = await drivetrainResponse.text();
        const drivetrainOptions = drivetrainText.split('\n').filter(drivetrain => drivetrain.trim());

        const engineResponse = await fetch('../public/options/car_CA_engine_size_values.txt');
        const engineText = await engineResponse.text();
        const engineOptions = engineText.split('\n').filter(engine => engine.trim());

        const transmissionResponse = await fetch('../public/options/car_CA_transmission_values.txt');
        ...
        new Dropdown('make', makeOptions);
        new Dropdown('model', modelOptions);
        new Dropdown('body_type', bodyOptions);
        new Dropdown('fuel_type', fuelOptions);
        new Dropdown('drivetrain', drivetrainOptions);
        new Dropdown('engine_size', engineOptions);
...
document.getElementById('prediction-form').addEventListener('submit', async function (e) {
    ...
    let engine_size = document.getElementById('engine_size').value
    let miles = document.getElementById('miles').value
    let fuel_type = document.getElementById('fuel_type').value
    const price = await fetchingalldata(make, model, year, body_type, transmission, drivetrain, engine_size, miles, fuel_type)
...
let pricing;
async function fetchingalldata(make, model, year, body_type, transmission, drivetrain, engine_size, miles, fuel_type) {
    ...
    try {
        const response = await fetch('/car/CA', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ make, model, year, body_type, transmission, drivetrain, engine_size, miles, fuel_type })
        });
