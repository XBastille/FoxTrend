...
class UsedCarPricePredictor:
    def __init__(self):
        self.model = joblib.load('model_ca/model.joblib')
        self.scaler = joblib.load('model_ca/scaler.joblib')
        self.feature_names = joblib.load('model_ca/feature_names.joblib')
        
        self.encoders = {}
        for feature in ['make', 'model', 'body_type', 'drivetrain', 'transmission', 'fuel_type']:
            self.encoders[feature] = joblib.load(f'model_ca/{feature}_encoder.joblib')
